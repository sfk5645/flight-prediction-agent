...
def test_weather_units_are_labeled():
    from flight_agent.serve.services import _annotate_weather_units

    out = _annotate_weather_units(
        {
            "airport": "IAD",
            "temperature_2m_mean": 23.1,
            "precipitation_sum": 0.0,
            "windspeed_10m_max": 5.8,
        }
    )
    assert out["units"]["temperature"] == "celsius"
    assert out["temperature_c"] == 23.1
    assert out["temperature_f"] == 73.6
    assert "23.1°C" in out["summary"]
    assert "73.6°F" in out["summary"]
...
def test_predict_delay_includes_minutes_when_regressor_present(tmp_path, monkeypatch):
    """Classifier + regressor artifacts → predicted_arr_delay_minutes on predict."""
    import json

    import numpy as np

    from flight_agent.features.build import FEATURE_COLUMNS
    from flight_agent.serve import services

    model_dir = tmp_path / "models"
    model_dir.mkdir()
    monkeypatch.setenv("FLIGHT_MODEL_DIR", str(model_dir))
    get_settings.cache_clear()
    services.load_model.cache_clear()
    services.load_meta.cache_clear()
    services.load_regressor.cache_clear()
        ...
        json.dumps(
            {
                ...
                "threshold": 0.5,
                "regression_clip_min": -30,
                "regression_clip_max": 240,
            }
        )
    )
    # Touch files so load_* path checks pass; actual load is stubbed below.
    (model_dir / "model.joblib").write_bytes(b"x")
    (model_dir / "model_regressor.joblib").write_bytes(b"x")
    ...
    monkeypatch.setattr(services, "load_model", lambda: _FixedClf())
    monkeypatch.setattr(services, "load_regressor", lambda: _FixedReg())
    monkeypatch.setattr(
        services,
        "_enrich_from_lake",
        lambda **kwargs: {
            "origin_hist_avg_taxi_out": 15.0,
            "origin_hist_avg_nas_delay": 5.0,
            "origin_hour_sched_flights": 40.0,
            "origin_hist_hour_pct_delay_15": 0.2,
            "route_hist_pct_delay_15": 0.25,
            "carrier_hist_pct_delay_15": 0.18,
            "is_peak_hour": 1,
            "distance": 800.0,
        },
    )
    monkeypatch.setattr(services, "get_weather", lambda *a, **k: {})

    out = services.predict_delay(
        op_unique_carrier="DL",
        origin="ATL",
        dest="LAX",
        fl_month=6,
    ...
    assert out["predicted_arr_delay_minutes"] == 22.5
    assert out["predicted_delay_15"] == 1
    assert "minutes" in out["interpretation"].lower()


def test_model_config_trains_regressor():
    cfg = load_project_config()
    assert cfg["model"].get("train_regressor") is True
    assert cfg["model"].get("regression_clip_min") == -30
    assert cfg["model"].get("regression_clip_max") == 240
